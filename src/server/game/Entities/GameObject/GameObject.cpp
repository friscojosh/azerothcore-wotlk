...
void GameObject::SaveToDB(uint32 mapid, uint8 spawnMask, uint32 phaseMask, bool saveAddon /*= false*/)
{
    ...
    // AzCodex: the INSERT above leaves zoneId/areaId out, so every save (.gobject add, .gobject move)
    // resets them to 0. Write them in the same transaction from where the object now stands.
    if (FindMap())
    {
